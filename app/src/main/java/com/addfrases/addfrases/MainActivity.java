        ...
        PostResponseAsyncTask task1 = new PostResponseAsyncTask(MainActivity.this, false, new AsyncResponse() {
            ...
            public void processFinish(String s) {
                ...
                for(User value: userList){
                    ...
                    UserCallBack.nameUser = value.name_user;
                    usuario.add(value.sobre_user);
                    UserCallBack.sobreUser = value.sobre_user;
                    usuario.add(value.foto_user);
                    UserCallBack.fotoUser = value.foto_user;
                    usuario.add(value.email_user);
                    usuario.add(value.background_user);
                    UserCallBack.backgroundUser = value.background_user;
                }
                ...
                username.setText(UserCallBack.nameUser+" "+"@"+UserCallBack.userName);
                status.setText(UserCallBack.sobreUser);

                AQuery aq = new AQuery(MainActivity.this); // intsialze aquery
                aq.id(R.id.profile_image).image(UserCallBack.fotoUser);
            }
        });
        task1.execute(url1);

        String url2 = "http://192.168.0.100/client/post.php?format=json";
